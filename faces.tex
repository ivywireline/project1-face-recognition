...
\[ J(\begin{bmatrix}
    \theta_{11}       & \theta_{12} & \theta_{13} & \dots & \theta_{1n} \\
    \theta_{21}       & \theta_{22} & \theta_{23} & \dots & \theta_{2n} \\
    \hdotsfor{5} \\
    \theta_{k1}       & \theta_{k2} & \theta_{k3} & \dots & \theta_{kn}
...
$$\frac{\partial J}{\partial \theta} = 2X(\theta^{T}X - Y)^{T}$$
\newline
...
def df_multiclass(x, y, theta):
    ...
    return -2*sum((y.T-dot((theta.T).T, x.T))*x.T, 1)
\end{lstlisting}
